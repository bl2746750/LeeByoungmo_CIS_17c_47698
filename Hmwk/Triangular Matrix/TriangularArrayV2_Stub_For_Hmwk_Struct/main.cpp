 ...
 * Created on September 9th, 2019, 23:15 pM
...
int main(int argc, char** argv) {
    ...
    triang.size=10;             //Literal column array size
    triang.col=fillAry(triang.size);       //Dynamic 1-D array
    triang.indx=filIndx(triang.size);      //Fill the index
    triang.data=fillAry(triang.col,triang.size);//Dynamic triangular array

    //Sort the columns
    markSrt(triang.col,triang.indx,triang.size);  //Sort with the index
    
    //Output Located Here
    cout<<"The original Column Matrix"<<endl;
    prntAry(triang.col,triang.size);       //Print the original col size for each row
    cout<<endl<<"The sorted Index Array"<<endl;
    prntAry(triang.indx,triang.size);      //Print the sorted index of column sizes
    cout<<endl<<"The unsorted Triangular Matrx"<<endl;
    prntAry(triang.data,triang.col,triang.size);//Print the Triangular matrix unsorted
    cout<<endl<<"The Triangular Matrix printed by smallest column to largest"<<endl;
    prntAry(triang.data,triang.col,triang.indx,triang.size);//Print the Triangular matrix sorted by col

    //Deallocate the array
    destroy(triang.data,triang.col,triang.indx,triang.size);//Clean up the results
