...
Dealer::Dealer(int num){
    ...
    //        cout << "ap=" << aP <<endl;
        ...
        for(int i=0;i<this->numPlayers;i++){
            ...
            cout << "Player" << order << ": " << this->players[order].getHandsName() << endl;
...
int Dealer::decideWinner(){
    int max=0;
    int winner=0;
    int nP=this->getNumPlayers();
    
    for(int i=0;i<nP;i++){
        if(this->players[i].getActStatus()==1) {
            if(this->players[i].getHands()>max) {
                max=this->players[i].getHands();
                winner=i;
            }
        }
    }
...
void Dealer::calBal(){
    int winner=this->decideWinner();
    cout << "Player" << winner << " won $" << this->getPotAmount() << "!" << endl;
    this->players[winner].addBal(this->getPotAmount());
    this->Dealer::resetPot();
}

int Dealer::resetGame(){
    int input;
    this->nextRound();
    
    for(int i=0;i<this->getNumPlayers();i++){
        
        this->players[i].resetHands();
    }
